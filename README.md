  ...
  (function() {
    ...
    function renderCatalog() {
      ...
      if (catalogProductsData.length === 0) {
        ...
        if (itemCount) itemCount.innerText = `0 Productos`;
        return;
      }
      ...
      if (itemCount) itemCount.innerText = `${catalogProductsData.length} Productos`;
    }
    ...
    window.openProductModalFromCard = function(cardElement) {
    ...
    function handleLogoClick() {
      ...
      clickTimer = setTimeout(() => {
        logoClicks = 0;
        if(clickBadge) clickBadge.classList.remove('visible');
      }, 4000);
    ...
    window.deleteCard = function(btn) {
      if (confirm('¿Eliminar este elemento del catálogo?')) {
        ...
        card.remove();
        saveAllData();
        if(itemCount) itemCount.innerText = `${document.querySelectorAll('.catalog-grid .card').length} Productos`;
      }
    }
